    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        {[
          ...
          <div key={stat.label} className="bg-[#141418] border border-white/10 p-5 rounded-3xl relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-3 opacity-5 group-hover:opacity-10 transition-opacity">
              <stat.icon className="w-8 h-8" />
            </div>
            <p className="text-[11px] font-bold text-zinc-300 uppercase tracking-widest mb-1">{stat.label}</p>
      ...
      <div className="flex-1 bg-[#141418] border border-white/10 rounded-3xl flex flex-col min-h-0 relative overflow-hidden">
        ...
        <div className="p-6 border-b border-white/10 flex items-center justify-between relative z-10">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-zinc-400 font-medium">Your latest calls</p>
            </div>
          </div>
          
          <button 
            disabled={!logs || logs.length === 0}
            className="flex items-center gap-2 px-4 py-2 bg-white/8 hover:bg-[#FEED01]/10 text-zinc-300 hover:text-[#FEED01] rounded-xl border border-white/10 hover:border-[#FEED01]/20 transition-all font-bold text-[11px] uppercase tracking-widest disabled:opacity-30"
          >
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar relative z-10">
          <table className="w-full text-left border-collapse">
            <thead className="sticky top-0 bg-[#141418] z-20">
              <tr className="border-b border-white/5">
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-[0.15em] text-zinc-400">Timestamp</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-[0.15em] text-zinc-400">Contact</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-[0.15em] text-zinc-400">Duration</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-[0.15em] text-zinc-400">Status</th>
                <th className="px-6 py-4 text-[11px] font-bold uppercase tracking-[0.15em] text-zinc-400">Result</th>
              </tr>
            </thead>
              ...
              {!logs || logs.length === 0 ? (
                ...
                logs.map((log) => (
                  <tr key={log.id} className="group hover:bg-[#FEED01]/5 transition-colors cursor-pointer">
                    <td className="px-6 py-4 text-[11px] font-bold text-zinc-300 font-mono">
                      {new Date(log.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}
                    </td>
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-8 h-8 rounded-full bg-zinc-700 flex items-center justify-center text-[10px] font-black text-white border border-white/10 group-hover:border-[#FEED01]/30">
                        ...
                        <div>
                          ...
                          <p className="text-[9px] text-zinc-500 font-bold">{log.contactPhone}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 text-[11px] font-bold text-zinc-300">
